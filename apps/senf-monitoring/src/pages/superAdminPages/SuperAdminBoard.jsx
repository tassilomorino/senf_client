...
import { Typography, ImagePlaceholder, Box, Icon, Table, Button, ModalButton, Input, isMobileCustom, ExpandMap } from "senf-atomic-design-system";
import {
  collection,
  deleteDoc,
  doc,
...
const SuperAdminBoard = () => {
  ...
  useEffect(() => {
    setFilteredMonitoringBoards(
      monitoringBoards.filter(monitoringBoard => Object.values(monitoringBoard).join(' ').toLowerCase().indexOf(searchTerm.toLowerCase()) > -1));
    ...
    <Wrapper>
      <Box gap="20px" flexDirection="column" margin="30px">
        ...
        <Box justifyContent="space-between" gap="16px" alignItems="flex-end">
          ...
          <ModalButton text="+ Create new monitoringBoard" options={{
            padding: 20,
        ...
        <Table
          ...
          columns={[
            ...
            { key: "monitoringBoardType", label: t('monitoringBoardType') },
            { key: "createdAt", label: t('createdAt') },
          ]}
        >
          {
            (row) => (
              <Box justifyContent="space-between" onClick={() => console.log("edit monitoringBoard now")}>
                <Box gap="16px">
                  {/* {!isMobile &&
                    <ImagePlaceholder
                      width="64px"
                ...
                <Typography variant="bodySm">{row.monitoringBoardType}</Typography>
