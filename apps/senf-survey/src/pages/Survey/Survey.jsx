...
import { Box, Button, LayerWhiteFirstDefault, ToggleInput, Typography } from "senf-atomic-design-system";
...
function Survey({ previewQuestions }) {
  ...
  const [questions, setQuestions] = React.useState(isPreviewing() ? previewQuestions : []);
        ...
        <Box height="100%" flexDirection="column" alignItems="center" justifyContent="center">
          <form onSubmit={handleSubmit}>
            {questions?.map((element, index) => {
              return (
                <Layer key={index}>
                  <Box padding="1rem 5rem 2rem 1rem" flexDirection="column" gap="1rem">
                    ...
                    <Box gap="0.5rem">
                      {element.options.map((option) => (
                        <React.Fragment key={option.uid}>
                          <Field
                            type={element?.qtype !== 'SingleChoice' ? "checkbox" : "radio"}
                            name={element.qtext}
                            id={option.uid}
                            value={option.value}
                            receiveValue={(value) => {
            ...
            <br />
            <Button disabled={isPreviewing()} variant="primary" type="submit" text="submit" />
          </form>
        </Box>
      )}
    </Formik>
