...
const ModelsList = ({
  ...
  const validationSchema = yup.object({
    ...
    imgURL: yup.string().required(t("add_img")),
    modelURL: yup.string().required(t("add_model")),
  });
  ...
  const handleUploadImageToFirestore = async (docId, formik) => {
    const options = {
      maxSizeMB: 0.03,
    ...
    const imgStorageRef = ref(
      storage,
      `threeD_models/${docId}/image/${formik.values.imgURL.name}`
    );
    const docRef = doc(db, `threeD_models/${docId}`);
  ...
  const handleModelUploadToFirestiore = async (docId, formik) => {
    console.log(docId);
    const storage = getStorage();
    const modelStorageRef = ref(
      storage,
      `threeD_models/${docId}/model/${formik.values.modelURL.name}`
    );
    const docRef = doc(db, `threeD_models/${docId}`);
    await uploadBytes(modelStorageRef, formik.values.modelURL).then(
      (snapshot) => {
        console.log("Uploaded a file!");
      }
    );
  ...
  const handleSubmit = async (event, formik) => {
    ...
    await addDoc(collection(db, "threeD_models"), data).then((docId) => {
      handleUploadImageToFirestore(docId.id, formik);
      handleModelUploadToFirestiore(docId.id, formik);
    });
  };
  ...
  const handlePlaceModel = (
    ...
    console.log("Creating model in handlePlaceModel", modelData);
  ...
  return (
    <Wrapper>
      <ThreeDToolSwipeList
        ...
        handlePlaceModel={handlePlaceModel}
        swipedUp={swipedUp}
