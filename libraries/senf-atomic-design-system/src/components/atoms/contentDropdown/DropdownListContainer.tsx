...
const Container = styled(animated.div)<DropdownListContainerProps>`
  ...
  top: ${({ options, theme }) =>
    options?.y === "down" ? `calc(100% + ${theme.space[2]})` : null};
  bottom: ${({ options, theme }) =>
    options?.y === "up" ? `calc(100% + ${theme.space[2]})` : null};
  left: ${({ options }) => (options?.x === "right" ? 0 : null)};
  right: ${({ options }) => (options?.x === "left" ? 0 : null)};

  transform-origin: ${({ options }) =>
    `${options?.y === "up" ? "bottom" : "top"} ${
      options?.x === "left" ? "right" : "left"
    }`};
