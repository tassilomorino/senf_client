...
const TableWrapper = styled.table<TableProps>`
  ...
  tbody {
    ...
    box-shadow: 0 0 0 2px ${({ theme }) =>
      theme.colors.greyscale.greyscale35tra};
  border-radius: 5px;
  border-spacing: 0; 
...
const Table: FC<TableProps> = ({
  ...
  const header =
    cols && (checked.length > 0 ? [bulkEdit, ...cols.slice(1)] : [...cols]);
  const allChecked =
    checked.length === 0
      ? false
      : data.every((item) => checked.includes(item[checkbox]))
      ? true
      : "indeterminate";
  const handleCheck = (checkboxId, value) => {
    setChecked(
      value
        ? [...checked, checkboxId]
        : checked.filter((item) => item !== checkboxId)
    );
  };
  ...
  useEffect(() => {
    setSortedData(
      orderBy(data, sortKey, sortDirection === false ? "asc" : "desc")
    );
                ...
                >
                  ...
                  <Icon
                    icon={
                      <Sort
                        topFill={
                          sortKey === col.key && sortDirection === false
                            ? theme.colors.black.black100
                            : theme.colors.black.black30tra
                        }
                        bottomFill={
                          sortKey === col.key && sortDirection === true
                            ? theme.colors.black.black100
                            : theme.colors.black.black30tra
                        }
                      />
        ...
        {sortedData?.length > 0 && typeof children === "function" && (
          <tbody>
            {sortedData.map((item, key) => (
              <TableRow
                key={key}
                checked={checked.includes(item[checkbox])}
