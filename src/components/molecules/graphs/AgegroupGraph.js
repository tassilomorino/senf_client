...
const AgegroupGraph = ({ classes, screams }) => {
  let topics_care;
  let topics_traffic;
  ...
  const votesTopic1agegroupUnder18 = votes.reduce((acc, cur) => {
  ...
  // TRYING TO ITERATE OVER AGEGROUPS

  // const agegroups = [18, 25, 35, 45, 55, 65, 100];

  // function votesTopic1(agegroups) {
  //   return votes.reduce((acc, cur) => {
  //     // cur refers to each object in the array votes & acc is a number that get returned

  //     for (const ages of agegroups) {
  //       if (listOfIdsPerTopic1.includes(cur.ideaId) && cur.age < ages) {
  //         return ++acc;
  //       } else {
  //         return acc;
  //       }
  ...
  useEffect(() => {
    ...
    // for (
    //   var i = 0;
    //   i < _.filter(agegroup18To24, { Thema: "Rad" }).length;
    //   i++
    // ) {
    //   let x = _.filter(agegroup18To24, { Thema: "Rad" })[i].likeCount;
    // }

    // var output = _(screams)
    //   .groupBy("Thema")
    //   .map((objs, key) => ({
    //     Thema: key,
    //     likeCount: _.sumBy(objs, "likeCount"),
    //   }))
    //   .value();
    ...
    // const total = Object.values(screams).reduce(
    //   (t, { likeCount }) => t + likeCount,
    //   0
    // );

    // const total = Object.values(screams).reduce((t, { likeCount }) => {
    //   "x" === "x" ? t + likeCount : 0;
    // });

    // var output = _(agegroup18To24)
    //   .filter(agegroup18To24, { Thema: "Rad" })
    //   .map((objs, key) => ({
    //     Thema: key,
    //     likeCount: _.sumBy(objs, "likeCount"),
    //   }))
    ...
    //   agegroup18To24,
    //   "sumby",
    //   _.sumBy(agegroup18To24, { Thema: "Rad" }, function (o) {
    //     return o.likeCount;
    //   })
    // );
